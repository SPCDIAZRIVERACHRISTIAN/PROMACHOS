        ...
        if looks_like_json {
            ...
            if let Ok(tc) = serde_json::from_str::<ToolCall>(&reply) {
                ...
                let tool_msg = format!(
                    "Tool `{}` result: {}",
                    out.name,
                    serde_json::to_string_pretty(&out.output)?
                );

                // Record the tool call and result into history
                history.push(ChatMessage { role: "assistant".into(), content: reply });
                history.push(ChatMessage { role: "user".into(), content: tool_msg });

                // second request, now that we have tool output
                ...
                let res2 = self.ollama.chat(&req2).await?;
                reply = res2.message.content;
            }
        }

        // Save assistant reply to history and return
